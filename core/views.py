...
def dashboard_responsive(request):
    ...
    tema_escuro = configuracao.dashboard_tema in ['corona-dark', 'vanta-dark']
...
def cartoes_dashboard(request):
    ...
    # Calcular dados de evolução temporal dos gastos (respeitando filtro de data)
    evolucao_labels = []
    evolucao_valores = []
    meses_ptbr = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez']
    if data_inicio and data_fim:
        ...
        hoje = timezone.now().date()
        data_inicio_dt = hoje.replace(month=1, day=1)
        data_fim_dt = hoje
    # Gera lista de meses entre data_inicio_dt e data_fim_dt
    meses_ano = []
    ano = data_inicio_dt.year
    mes = data_inicio_dt.month
    while (ano < data_fim_dt.year) or (ano == data_fim_dt.year and mes <= data_fim_dt.month):
    ...
    context = {
        ...
        'today': hoje,
        'fatura_id_selecionada': request.GET.get('fatura'),  # Adicionado para seleção visual
    }
...
@login_required
def cartoes_dashboard_ajax(request):
    data_inicio = request.GET.get('data_inicio')
    data_fim = request.GET.get('data_fim')
    cartao_id = request.GET.get('cartao')
    filtros_despesa = {'ativo': True}
    ...
    html = render_to_string('core/partials/cartoes_dashboard_painel.html', context, request=request)
...
class LoginViewCustom(LoginView):
    ...
    def form_valid(self, form):
        ...
        tema_login = self.request.POST.get('tema_login')
        user = self.request.user
        if user.is_authenticated and tema_login:
            try:
                configuracao = user.configuracao
                # SEMPRE atualiza o tema com o valor do login
                if tema_login in dict(Configuracao._meta.get_field('dashboard_tema').choices):
                    configuracao.dashboard_tema = tema_login
                    configuracao.save(update_fields=['dashboard_tema'])
